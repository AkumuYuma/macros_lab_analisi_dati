...
void psi_fit() {
    ...
    RooPlot * xFrame{x.frame(Title(""))}; // Vedere che fa
    // Nota che il frame lo estrai dalla variabile aleatoria
    ...
    RooGaussian g{"g", "Gauss(x, mean, sigma)", x, mean, sigma};
    ...
    totalPdf.paramOn(xFrame, Parameters(RooArgSet(meanCB, sigmaCB, nSig, mean, sigma, nSigGaussiana)), Layout(0.45, 0.9, 0.9)) ; // Stampo anche i valori per la gaussiana
