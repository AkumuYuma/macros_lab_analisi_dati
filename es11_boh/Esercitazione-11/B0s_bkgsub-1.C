...
int sidesub()
{
  ...
  //
  myc.SaveAs("B0s_and_Phi_Mass_before_cut.png");
  //
  /////////////////////////////////////////////////////////////////////////////
  //
  ...
  myc2.SaveAs("B0s_and_Phi_Masses_after_displacement-cut.png");
  //
  ...
  //
  //
  //myc1.SaveAs("b0sMass_NONprompt_only.png");
  //--> note that the RooPlot has binning 200 (bin width=3.5MeV), the RooHist has now also 200 bins [from 100 (bin-width=7MeV)]
  //let's fit the Histogram
  //
  ...
  RooGaussian gauss1("gauss1","Gauss(mVar,mg,wg)",mVar,mg,wg);
  //
  ...
  //RooRealVar c3("c3","4th coeff",0.2, -1000.,1000.); // not needed - no real gain
  ...
  //B0s_model_ext->fitTo(*B0sMass_aftercut_200, Extended(kTRUE), Minos(kTRUE)); 
  ...
  cout << "################ PURITY = S/S+B = SIGNAL/SIGNAL+BKG = " << purity.getVal() << endl;
  //
  //lineSdbUPup->plotOn(mVarFrame);                  
  mVarFrame->Draw();
  ...
  //RooRealVar cc3("cc_{3}", "cc3", 0.1 ,-10, 10); //  good but still enhances the nSig yiel
  //RooRealVar cc4("cc_{4}", "cc4", 0.1 ,-10, 10); // good but still enhances the nSig yiel
  //RooRealVar cc5("cc_{5}", "cc5", 0.05 ,-10, 10); //--not needed (too many DOF)   
  //RooRealVar cc6("cc_{6}", "cc6", 0.0 ,-1, 1); //--not needed
  //
  ...
  mPhiVarFrame->Draw();
  //
  myc1.SaveAs("B0sPhiMass_NONpromptOnly.png");
  //
  return 0;
}
